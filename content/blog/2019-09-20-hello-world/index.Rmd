...
I have always wanted to start a blog since graduate school when I read [Daniel Lakens' post](http://daniellakens.blogspot.com/2017/04/five-reasons-blog-posts-are-of-higher.html) on why blog posts may have higher quality than scientific journals.{{%mn%}}which I highly recommend you have a read{{%/mn%}}
...
Now, this creates another issue.
...
I try to strip out this *style* chunk with `replaceRE` and `plainify` [but that didn't work](https://github.com/gohugoio/hugo/issues/3235).
Finally, Yihui Xie's [post](https://yihui.name/en/2017/08/hugo-post-summary/#fn:Why-Scratch-Beca) on how to generate a Hugo summary came to the rescue.
Following Yihui's guide gets me very close but still not quite what is expected.
Finally, 林志扬's incredible insight:

> I suggest using `(.|\n)*?` instead of `.*?` when searching for paragraph contents, since a single line break won't start a new paragraph.

saves the day.
I probably should not have spent this much time on syntax highlighting, but I desperately preferred *some* highlighting to none at all.
...
![the bike crew before heading back to Philly from Bear Creek (we did 120+ miles in total!) I still need to photoshop in our last biker [Bill La Cava](http://williamlacava.com/))](bike-crew.jpg)
